...
int main(int argc, char** argv) {
    try {
        ...
        std::cout << "Building Faiss IndexHNSWFlat: d=" << base.columns
                  << ", nb=" << base.rows << ", M=" << options.M
                  << ", efConstruction=" << options.ef_construction << std::endl;
        faiss::IndexHNSWFlat index(
                static_cast<int>(base.columns), options.M, faiss::METRIC_L2);
        ...
        raw << "ef_search,repeat,elapsed_seconds,qps,top_k,recall_at_k,threads,M,"
               "ef_construction,query_count,build_seconds\n";
        summary << "ef_search,median_qps,top_k,recall_at_k,threads,M,ef_construction,"
                   "query_count,repeats,build_seconds\n";
